...
### 第二节 CentOS7系统 编译安装 Python3 环境

#### 1.2.1 准备编译环境
&emsp;&emsp; 编译安装 Python之前，我们还需要准备好编译环境。下面我们在CentOS7  
系统中安装具体编译软件和 python 依赖的软件包，如下：
```bash
...
#### 1.2.2 获取 python3 源码包

&emsp;&emsp; 访问 [Python](https://www.python.org) 官方网站，下载指定版本的源代码包, 在网站 Download   
导航栏中选择 Source code ，然后在跳转的页面中选择自己想要的 python  
版本，这里我们复制下载地址，使用 wget 命令下载 python3.7.3，如图：
```bash
